...
class IPCServer:
    ...
    async def broadcast_foreign_speech(
        ...
        if vr_overlay is not None:
            asyncio.create_task(
                self._push_vr(vr_overlay, source_text, detected_language, translation)
            )
    ...
    async def _push_vr(
        ...
        try:
            await vr_overlay.push(source_text, detected_language, translation)
        except Exception as exc:
