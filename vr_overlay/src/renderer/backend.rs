...
#[cfg(windows)]
use super::types::{
    contains_cjk, effective_background_alpha, fill_color_for_channel,
    secondary_fill_color_for_channel, outline_offsets_px,
    BlockBounds, BlockCacheKey,
    CaptionBlockVariant, CaptionDebugOverlay, CaptionChannel, CaptionLayoutResult,
    CaptionPresentation, CaptionRenderError, DamageBand, LineCacheKey, LineRole,
    RenderDiagnostics, ResolvedBlockLayout,
    ResolvedFrameLayout, ResolvedLineLayout, ResolvedTextStyle, StyleBucketSourceCount,
    TextStyleDescriptor, DEFAULT_FONT_SIZE_PX, DEFAULT_SURFACE_HEIGHT_PX,
...
impl WindowsCaptionRenderer {
    fn new() -> Result<Self, CaptionRenderError> {
        ...
        let cache_peer_secondary_text_brush = unsafe {
            d2d_context
                .CreateSolidColorBrush(
                    &d2d_color(secondary_fill_color_for_channel(CaptionChannel::PeerChannel)),
                    None,
                )
                .map_err(|error| CaptionRenderError::Init(error.to_string()))?
        };
