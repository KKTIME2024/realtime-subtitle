...
impl OverlayRuntime {
    ...
    pub fn caption_blocks(&self) -> Vec<CaptionBlock> {
        self.state
            .scene()
            .occupied()
            .map(PresentationSlot::to_caption_block)
            .collect()
    }
    ...
    pub fn apply_snapshot(&mut self, snapshot: OverlayPresentationSnapshot) -> SnapshotApplyOutcome {
        ...
        for slot_index in 0..self.state.scene().slots().len() {
            let previous = match self.state.scene().slots().get(slot_index).and_then(Option::as_ref) {
                Some(previous) => previous,
                None => continue,
            };
            if let Some(found) = incoming
                .iter()
                .position(|b| {
                    b.occupant_key == previous.occupant_key && b.appearance_seq == previous.appearance_seq
                })
            {
        ...
        for (i, block) in incoming.iter().enumerate() {
            ...
            let free_index = (0..VISIBLE_SLOT_CAP).find(|idx| scene.slots().get(*idx).and_then(Option::as_ref).is_none());
    ...
    pub async fn handle_event(&mut self, event: OverlayBridgeEvent) -> Result<(), RuntimeFailure> {
        match event {
            ...
            OverlayBridgeEvent::Captions(update) => {
                if !update.self_text.is_empty() || !update.peer.is_empty() {
                    let block = crate::state::OverlayPresentationBlock {
                        ...
                        channel: if !update.self_text.is_empty() { "self" } else { "peer" }.into(),
            ...
            OverlayBridgeEvent::Control(_) | OverlayBridgeEvent::Heartbeat
...
impl PresentationSlot {
    fn from_block(block: &crate::state::OverlayPresentationBlock, slot_index: usize, entry_order: u64) -> Self {
        Self {
            slot_index,
...
impl crate::openvr::OverlayFrameSubmitter for ShellSubmitter<'_> {
    fn submit_frame(&mut self, frame: &crate::renderer::RenderedFrame) -> Result<(), crate::openvr::OpenVrError> {
