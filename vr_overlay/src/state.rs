...
fn default_anchor() -> String { "head_locked".into() }
fn default_offset_y() -> f32 { -0.45 }
fn default_distance() -> f32 { 1.1 }
fn default_text_scale() -> f32 { 1.0 }
fn default_background_alpha() -> f32 { 0.24 }
...
const SLOT_ROW_STRIDE_PX: f32 = 504.0;
/// Default top position for the first slot.
const FIRST_SLOT_TOP_PX: f32 = 40.0;
...
impl OverlayState {
    ...
    pub fn apply_snapshot(&mut self, incoming: &OverlayPresentationSnapshot) -> bool {
        ...
        for (i, block) in blocks.iter().enumerate() {
            ...
            let free_index =
                (0..VISIBLE_SLOT_CAP).find(|idx| scene.slots().get(*idx).and_then(Option::as_ref).is_none());
            ...
            let slot = OverlayStateSlot::from_block(block, slot_index, self.entry_counter, anchor_top_px);
    ...
    pub fn blocks(&self) -> Vec<OverlayPresentationBlock> {
        self.scene.slots().iter().flatten().map(ToPresentationBlock::to_block).collect()
    }
