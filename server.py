...
def main():
    ...
    web_server.overlay_manager = DesktopOverlayManager(f"http://127.0.0.1:{actual_port}", web_server=web_server)

    # ── VR overlay 管理器 (Settings 开关热切换) ──
    vr_exe = config.VR_OVERLAY_EXE or str(
        Path(__file__).resolve().parent / "vr_overlay" / "target" / "release" / "RinBridgeOverlay.exe"
    )
